...
export const images = {
  ...
  /**
   * Кадр сцены: кабинет без людей — первый план анимации.
   * alt пустой намеренно: это монтажный переход к тому же кадру, и уже
   * через секунду его перекрывает следующий. Со своим описанием он заставлял бы
   * скринридер зачитывать одну сцену дважды, начиная с невидимой картинки.
   */
  sceneEmpty: {
    name: 'scene-empty',
    widths: [640, 900],
  ...
  /** Тот же кабинет, но в кадре появляется человек */
  sceneHer: {
    name: 'scene-her',
    widths: [640, 900],
    ratio: 3 / 4,
    alt: 'Юлия Радионова в кабинете',
  },
  /**
   * Кабинет в расфокусе — задний план полноэкранной сцены.
   * Размытие и затемнение впечены в файл на сборке, alt пустой: смысла
   * без переднего плана он не несёт.
   */
  office: {
    name: 'office-blur',
    widths: [900, 1300],
  ...
  /** Крупный план поверх кабинета — «она здесь, только ближе к нам» */
  close: {
    name: 'scene-close',
    widths: [560, 820],
    ratio: 4 / 5,
  ...
  /**
   * Передний план сцены — размытая поверхность у нижнего края.
   * За ней скрывается место, где вырезанный кадр обрывается.
   */
  foreground: {
    name: 'foreground',
    widths: [900, 1300],
